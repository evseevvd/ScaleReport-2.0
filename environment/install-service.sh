...
if id ${USER} >/dev/null 2>&1; then
        echo "\e[41mInstall service fail: user exists.";
else
        echo "\e[34mCreate a new user 'reporter'";
        useradd reporter;
        echo "\e[34mCreate work directory";
        mkdir ${workdir};
        echo "\e[34mCopy files...";
        ...
        echo "\e[34mAdd new service 'scale-server'";
        cp scale-server.service /etc/systemd/system;
        systemctl daemon-reload;
        systemctl enable scale-server;
        echo "\e[34mStart service 'scale-server'";
        systemctl start scale-server;
        echo "\e[34mSuccess. Start 'systemctl start scale-server' Stop 'systemctl stop scale-server'";
